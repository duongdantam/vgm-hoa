...
export const getThumbnailUrl = async (ipfsGW: string, cloudGW: string, item: Item, size: number = 480, select: number = 1) => {
	return new Promise<string>(async (resolve) => {
		...
		if (url && hash) {
			...
			const thumbUrl = await fetchTimeout(ipfsThumbUrl).then(res => res.status === 200 ? ipfsThumbUrl : cloudThumbUrl).catch(err => cloudThumbUrl);
			resolve(thumbUrl)
		} else {
...
export const getPlayUrl = async (ipfsGW: string, cloudGW: string, item: Item, isVideo: boolean = true) => {
	return new Promise<string>(async (resolve) => {
		// // get playUrl from cloud
		const itemPath = item.url.replace(/\./g, '/');
		...
		if (url && hash) {
			...
			const ipfsUrl = `${ipfsGW}/ipfs/${streamHash}/${isVideo ? 'playlist.m3u8' : '128p.m3u8'}`;
			// console.log('ipfsUrl:', ipfsUrl);
			const playUrl = await fetchTimeout(ipfsUrl).then(res => res.status === 200 ? ipfsUrl : cloudUrl).catch(err => cloudUrl);
			resolve(playUrl)
		} else {
			resolve(cloudUrl);
		}
