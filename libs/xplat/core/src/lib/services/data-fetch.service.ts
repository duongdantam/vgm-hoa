...
export class DataFetchService {
	...
	async init() {
		...
		await fetchAndActivate(remoteConfig)
			.then(() => {
				...
				this.cloudGateway = 'https://cdn.vgm.tv/encrypted' || getValue(remoteConfig, "CLOUD_GATEWAY").asString();
		...
		return new Promise(async (resolve) => {
			const recurse = async (item) => {
				...
				if (item.isLeaf === true || item.isLeaf === false) {
					await this.fetchTopicList(item.url).then(list => { if (list.children[0]) recurse(list.children[0]); });
				}
			}
			recurse(item);
		})
	...
	async fetchTopicList(topicUrl: string) {
		...
		if (!cacheList) {
			...
			if (/^(01-bai-giang.hoc-theo-sach-trong-kinh-thanh).*/.test(list.url) || /^(phat-thanh-nguon-song).*/.test(list.url)) expiredTime = 43200; // 172800
	...
	async fetchItemList(topicUrl: string) {
		...
		if (!cacheList) {
			...
			if (/^(01-bai-giang\.hoc-theo-sach-trong-kinh-thanh).+/.test(list.url) || /^(phat-thanh-nguon-song\.nam\-\d+\.thang).+/.test(list.url)) expiredTime = 43200; // 86400
			list.children.sort(function (a, b) {
				return parseInt(a.name.match(/^\d+/)) - parseInt(b.name.match(/^\d+/));
			});
			await this.localforageService.set(topicUrl, list, expiredTime);
	...
	async getPlayUrl(item: any, isVideo: boolean = true) {
		return await this.vgmCore.getPlayUrl(this.streamGateway, this.cloudGateway, item, isVideo);
	}
}
