...
import { PlayerService, BaseComponent, DataFetchService, Item, ItemList, LocalforageService, OfflineService, QueueService } from '@fy/xplat/core';
...
export class AudioPlayListComponent extends BaseComponent implements OnInit, OnDestroy {
	...
	async ngOnInit() {
		...
		this.favoriteSub = this.playerService.favoriteAudioPlaylist$.subscribe((state) => {
		...
		this.itemUrl$.subscribe(async (param) => {
			if (param) {
				console.log(this.activatedRoute.snapshot.params.topicUrl, this.router.url);
	...
	updateMetaTag(title, url) {
		...
		this.meta.updateTag({ property: 'og:title', content: `${title} - FUYIN TV` });
		this.meta.updateTag({ property: 'og:description', content: `${title} - description` });
		this.meta.updateTag({ property: 'og:image', content: `https://vgm.tv/assets/imgs/logo-vgm.svg` });
	}

	async checkDownloaded() {
		this.downloadedList = await this.offlineService.db.manifest.where("type").equals("audio").primaryKeys();
	}
	...
	async onDownload(item) {
		try {
			...
			if (!getKey) {
				...
				await this.downloadData(itemDir, path.basename(playUrl), m3u8AB, 'master', item);
				...
				this.queueService.aDownloadQueue.on('idle', async () => {
					if (downloadedFile === m3u8.segments.length) {
						await this.offlineService.db.manifest.put({ id: item.id, type: 'audio', metadata: item });
						await this.checkDownloaded();
						await this.presentToast(this.translateService.instant('msg.audio.download'));
	...
	addToFavorite(item: Item) {
		const itemIndex = this.favoriteList.findIndex(list => list.id === item.id);
		if (itemIndex >= 0) {
