...
export class VideoPlayerWidgetComponent
	...
	async ngOnChanges(changes: SimpleChanges) {
		if ('widgetLocation' in changes) {
			...
			if (this.widgetLocation === 0) {
				...
				this.videoPlayer.style.paddingTop = `${(56.25 * (this._minimizeHeight + this._offsetHeight)) /
					this.windowHeight
					}%`;
			}
		...
		if ('options' in changes && this.player) {
			...
			try {
				...
				this.player.play();
				this.updateMetaTag();
				document.title = `${this.playerService.isVideoPlaying
	...
	async ngOnInit() {
		...
		this.player.on('play', () => {
			...
			this.playingState.item.isVideo ? this.videoIsPlaying = true : this.audioIsPlaying = true;
			// if (!this.playingState.isPlaying && !this._seeking) {
			this.playingState.item.isVideo ? this.playerService.videoResume() : this.playerService.audioResume();
			// }
		...
		this.player.on('pause', () => {
			this.playingState.item.isVideo ? this.videoIsPlaying = false : this.audioIsPlaying = false;
			if (this.playingState.isPlaying && !this._seeking) {
				// console.log('video isplaying', this.playerService.videoPlayState.isPlaying);
				this.playingState.item.isVideo ? this.playerService.videoPause() : this.playerService.audioPause();
			}
			if (this.platform.is('capacitor') && this.isHidden === false) {
				CapacitorMusicControls.updateIsPlaying({
		...
		this.player.on('seeking', async () => {
			const preFetchUrl = `${path.dirname(this.options.sources[0].src)}/${this.player.qualityLevels().levels_[
	...
	updateMetaTag() {
		...
		const playingUrl = `tabs/${this.playingItem.isVideo ? 'video' : 'audio'
	...
	async onDownloadSingleSelector(item) {
		...
		if (!getKey) {
			...
			if (selectedQuality.role !== 'backdrop' && selectedQuality.role !== 'cancel') {
				item.isVideo ? await this.offlineService.onVideoDownload(item, selectedQuality.role) : await this.offlineService.onAudioDownload(item);
				this.isDownloaded = await this.offlineService.checkMediaIsDownloaded(this.playingItem);
			}
		} else {
			...
			if (confirmRemove.role === 'ok') {
				...
				this.isDownloaded = await this.offlineService.checkMediaIsDownloaded(item);
				if (item.isVideo && !this.isDownloaded && this.queueService.downloadProgress === 0) {
					this.presentToast(this.translateService.instant('msg.video.delete'))
				};
				if (!item.isVideo && this.offlineService.offlineAudioPlaylistID.indexOf(item.id) < 0) {
	...
	async onDownloadTopicSelector(topic) {
		topic.size = topic.children.reduce((accumulator, currentValue) => accumulator + currentValue.size, 0);
		console.log("download topic::", topic);
		const selectedQuality = await this.offlineService.downloadMediaAlertConfirm(topic);
		console.log("on selectedQuality::", selectedQuality.role);
		if (selectedQuality.role !== 'backdrop' && selectedQuality.role !== 'cancel') {
			for await (const item of topic.children) {
				await this.offlineService.onVideoDownload(item, selectedQuality.role)
			}
		}
	}
	...
	onTouchMove(event) {
		if (!this._seeking) {
			...
			if (Math.abs(this.movementPos) > 15) {
				...
				this.videoPlayer.style.paddingTop = `${(56.25 * (this.windowHeight - translatePos + this._offsetHeight)) /
					this.windowHeight
					}%`;

				this.videoInfo.style.marginTop = `calc(100vw * ${(0.5625 * (this.windowHeight - translatePos + this._offsetHeight)) /
					this.windowHeight
					})`;
			}
		}
