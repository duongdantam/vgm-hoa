...
import { IonSearchbar, Platform, AlertController, ToastController, ModalController } from '@ionic/angular';
...
export class MobilePageHeaderComponent extends BaseComponent implements OnInit {
	...
	constructor(
		...
		this.router.events.subscribe(async (event) => {
			if (
				...
				event.url.split('/')[3] &&
				(event.url.split('/')[3].startsWith('favorite') ||
					event.url.split('/')[3].startsWith('download'))
			) {
	...
	async searchChange(e) {
		...
		await Promise.all(
			this.searchResult.hits.map(async (item) => {
				...
				item.thumb = this.isVideo ? await this.getItemThumbnail(item) : null;
	...
	public searchMore(param) {
		this.router.navigate(
			['/muc-luc', this.isVideo === true ? 'video' : 'audio', 'search'],
			{
	...
	async openNotification() {
		await this.presentModal(this.dataFetchService.desktopAppUrl, this.dataFetchService.notificationList)
	}

	async presentModal(url: string, arr) {
		const modal = await this.modalController.create({
			component: NotificationModalComponent,
			cssClass: 'notification-modal',
			enterAnimation: rightToLeftEnter,
			leaveAnimation: rightToLeftLeave,
			animated: true,
			swipeToClose: true,
			componentProps: {
				'desktopAppUpdateUrl': url,
				'notificationArr': arr
			}
		});
