...
export class TabsPage implements OnInit, OnDestroy {
	...
	public menuActivation = { video: false, audio: false, favorite: false, document: false };
	constructor(
		...
		// video
		this.videoSub = this.playerService.videoPlayState$.subscribe(async (state) => {
			if (state.isPlaying) {
				this.videoPlayUrl = await this.dataFetchService.getPlayUrl(state.item, true);
		...
		// audio
		this.audioSub = this.playerService.audioPlayState$.subscribe(async (state) => {
			if (state.isPlaying) {
				this.audioPlayUrl = await this.dataFetchService.getPlayUrl(state.item, false);
	...
	async ngOnInit() {
		...
		this.audioRootList = await this.dataFetchService.fetchRoot('audio').then(async (list) => {
			if (list) {
		...
		return function (menu) {
			...
			if (menu === 'video') {
				...
				if (this.menuActivation.video === false) {
					...
					this.router.navigate(['/tabs', menu, 'catalog'], { queryParams: { topicUrl: this.videoRootList[0].url } });
				}
				...
				if (this.menuActivation.audio === false) {
					...
					this.router.navigate(['/tabs', menu, 'catalog'], { queryParams: { topicUrl: this.audioRootList[0].url } });
				}
			...
			// thís code is for mobile 
				...
				if (this.menuActivation.favorite === false) {
					...
					this.router.navigate(['/tabs', menu, 'playlist'], { queryParams: { id: 'video' } });
				}
			}
