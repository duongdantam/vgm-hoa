...
export class AudioPlaylistPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (topicUrl) {
      ...
      if (topicData.children) {
        ...
        // sort children with name
        topicData.children.sort(function (a, b) {
          return (
      ...
      this.itemUrl$.subscribe(async (param) => {
        if (param) {
          ...
          const index = this.topicCategory.children.findIndex(
            (list) => list.url === url
          );
  ...
  private getItemHref(item: any) {
    return ['/tabs', 'audio', item.isLeaf ? 'playlist' : 'catalog', item.url];
  }
  loadMoreData(event) {
    setTimeout(() => {
