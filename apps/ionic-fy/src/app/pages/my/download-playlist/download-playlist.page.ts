...
export class DownloadPlaylistPage implements OnInit {
	...
	constructor(
		...
		this.offlineVideoSub = this.offlineService.offlineVideoPlaylist$.subscribe((data) => {
			const list = _.mapValues(_.groupBy(data, 'pname'), clist => clist.map(list => _.omit(list, 'pname')));
			if (!_.isEmpty(list)) this.offlineVideoList = list; else this.offlineVideoList = null;
		});

		this.offlineAudioSub = this.offlineService.offlineAudioPlaylist$.subscribe((data) => {
			const list = _.mapValues(_.groupBy(data, 'pname'), clist => clist.map(list => _.omit(list, 'pname')));
			if (!_.isEmpty(list)) this.offlineAudioList = list; else this.offlineAudioList = null;
		});
	...
	selectTab(tabIndex: number) {
		setTimeout(function () {
			let element: HTMLElement = document.getElementById('document-tab' + tabIndex) as HTMLElement;
			element.click()
		}, 10);
	}
